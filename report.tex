...
\label{sec:tagme}
We have used \href{http://tagme.di.unipi.it/}{TAGME} to annotate tweets, associating some topics to each tweets of the dataset. The topics is associated to the tweets only of the annotation has rho greater than a default threshold.
...
\label{sec:merge}
One fundamental step in the previous procedure is how to merge together informations coming from different users into a single score. We assumed three possible score measures that are very simple to calculate and give a base for furter reasoning. They are, for a fixed pair $(h, a)$ (hashtag, annotation):
\begin{enumerate}
\item the user count $u(h,a)$, that is simply the number of users that used the pair $(h,a)$ in at least one tweet, indifferently from the number of these tweets. Having a high $u(h,a)$ is an indication of the popularity of the pair $(h,a)$: the hashtag $h$ is used by many users with meaning $a$.

\item the total number of tweets where the $(h,a)$ pair is used, disregaring any information about the users (i.e. the previous indicator). We will indicate this number with $f(h,a)$. This can be easily calculated if $w(u,h,a)$ simply represent the number of tweets written by $u$, where the pair $(h,a)$ is used. In formulas:
\begin{equation}
\label{eqn:frequency}
f(h,a) = \sum_{u} f(u,h,a)
\end{equation}
...
\item the total sum of the \textit{rho}s if $w(u,h,a)$ is instead based on a $\rho$-weighting. This number will be indicated with $P(h,a)$ in the following part.
\begin{equation}
P(h,a) = \sum_{u} P(u,h,a)
\end{equation}
\end{enumerate}
...
The measure $f(h,a)$ (equation \ref{eqn:frequency}) is similar but with a linear function in place of $\boldsymbol{1}_{(0,\infty)}(x)$.
...
\item $f(h,a)$ (or $P(h,a)$) shouldn't be negligible. In other words the number of users that used association $(h,a)$ is clearly important, but there should be also some dependance, in the score, on how many times (or how often) a certain user used that association.
...
A possible idea is to use a different function in equation \ref{eqn:step} (something between a step function and a linear function) in order to get a measure that is closer to $ u(h,a) $ but increasing with $ f(h,a) $. A good score measure can be:
...
Notice that if the logarithm in equation \ref{eqn:score} is in base 2 and the weights used are the number of tweets (so $w(u,h,a) = f(u,h,a)$), the result of this score is always between $u(h,a)$ and $f(h,a)$.
In fact the argument of the logarithm will be one if there are no tweets for user $u$ containing the hashtag $h$ with meaning $a$, otherwise it will have always a value greater or equal to two, since $f(h,a)$ (for user $u$) will be always greater or equal to one.

\begin{table}[h]
\centering{
\begin{tabular}{c | c | c | c}
%\hline
example of $f(u,h,a)$ weights for 20 users & $u(h,a)$ & $\score(h,a)$ & $f(h,a)$\\
\hline
...
If the definition doesn't exist or isn't good, the server allows you to create your own definition in few seconds. So it's possible to obtain up to three definitions for an hashtag, sorted by the votes of users. All twitter-users can contribute to grow the database of definitions (as in Wikipedia).
