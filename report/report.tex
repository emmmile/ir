...
\item Merge all the informations produced above creating a unique score for association \ha, summing up the contributions of the various users. This has been done with the script \texttt{hashtagMerge.py}. Here the annotations are sorted by their importance, as explained in section \ref{sec:merge}.
\end{enumerate}
...
We plotted more graphs of hashtags sorted by frequency, trascribing the frequency (in red) and the number of users those using that hashtag (in green) (see table \ref{fig:plotFrequency}).
...
\begin{tikzpicture}
    \begin{axis}[
		...
		\addplot[color=red,mark=*,only marks]   table[x=rank,y=users] {plot/frequency-score-users-500.plot};
		\addlegendentry{hashtag users}
		\addplot[color=green,mark=x,only marks]   table[x=rank,y=score] {plot/frequency-score-users-500.plot};
		\addlegendentry{hashtag score}
		\addplot[color=blue]   table[x=rank,y=frequency] {plot/frequency-score-users-500.plot};
		\addlegendentry{hashtag frequency}
    \end{axis}
\end{tikzpicture}
\caption{The ranking of the best \ha pairs in terms of frequency, showing also the other related 2 measures introduced in the previous sections (this cause the big fluctuations of the score and the number of users). The range of the plot has been restricted to $[0,10^4]$ for better viewing.}
...
\begin{tikzpicture}
    \begin{loglogaxis}[
		...
		\addplot[color=red]   table[x=rank,y=frequency] {plot/frequency-score-users-1000.plot};
		\addlegendentry{hashtag frequency}
		\addplot[color=blue,dashdotted,mark=none] table[x=rank,y={create col/linear regression={y=frequency}}] {plot/frequency-score-users-1000.plot};
...
raiperunanotte & \footnotesize{Italian streamed and free version of a TV talk show called \#annozero, which was instead censored by \#berlusconi. On march\dots} & \footnotesize{\# let it not be for one night only \# let it be freedom for all and forever} & \footnotesize{Raiperunanotte is the first step to restore democracy, freedom and justice in Italy. Thanks to the web\dots}\\
 \hline
    \end{tabular}
    \caption{Table of the first three definitions of some hashtags. To note that the definitions are written in many languages (english, spanish, italian\dots).}
    \label{tab:hashDef}
\end{table}
...
\centering
    ...
    \hline
Tweet & Hashtags sorted by the score (shows for the first two)\\ \hline
Berlusconi è l'ex-presidente del consiglio & \footnotesize{\#berlusconi s=2935 \#annozero s=377 \#ballar \#pdl \#laresadeiconti \#italy \#politica \#finecorsa \#italia \#rubygate}\\
Monti giura e nomina il nuovo governo & \footnotesize{\#governo s=345 \#berlusconi s=227 \#italia \#politica \#monti \#fiducia \#pdl \#opencamera \#referendum \#annozero}\\
Grillo stravince a Parma, l'antipolitica prende campo & \footnotesize{\#parma s=456 \#annozero s=150  \#grillo \#parmalat \#notav \#m5s  \#juventus \#beppegrillo \#beppe \#calcio}\\
\hline
    \end{tabular}
    \caption{Examples of tagger use. The hashtags are sorted by score. At every tweets can be associated more annotations, then the found hashtags range over the relatived topics .}
    \label{tab:tagger}
\end{table}
